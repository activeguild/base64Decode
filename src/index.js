...
const base64Decode = (input) => {
    ...
    const binaryNums = [...replacedInput].reduce((prev, curr) => {
        return `${prev}${conversionMap.get(curr) || ""}`;
    }, "");
    const split4Len = binaryNums.match(/.{1,4}/g);
    return sliceByNumber(split4Len, 2).reduce((prev, curr) => {
        if (curr.length !== 2)
            return prev;
        const ascii = `${parseInt(curr[0], 2).toString(16)}${parseInt(curr[1], 2).toString(16)}`;
        return `${prev}${String.fromCharCode(parseInt(ascii, 16))}`;
    }, "");
};
const sliceByNumber = (array, length) => {
...
assert_1.default.strictEqual(base64Decode("QUJDREVGRw=="), "ABCDEFG");
assert_1.default.deepStrictEqual(base64ToUint8Array("QUJDREVGRw=="), [65, 66, 67, 68, 69, 70, 71]);
